...
def PrintRecBlockSpecial(X,Y,filepath):
    ...
    try:
        with open(filepath+"/指示.txt","w",encoding='utf8') as f:
...
def Detect(filepath):
    ...
    global startY
    try:
        with open(filepath+"/tmplog.log","r",errors='ignore') as f:
            con = f.readlines()
...
if __name__ == "__main__":
    ...
    currentX = 0
    currentY = 0
    BlockList = GenGeneralBlockList(r"pixel/鹡鸰.mcfunction")
    while True:
        Detect(r"C:/Users/Administrator/Documents/tmpupload")
        time.sleep(1)
